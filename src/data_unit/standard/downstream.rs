...
impl DataUnit for ReadDeviceOperation {
    ...
    fn parse(data: &[u8]) -> ParseResult<Self> {
        ...
        Self::new(record_count, start_time).map_err(|e| ParseError::InvalidValue {
            field: "record_count".to_string(),
            value: record_count.to_string(),
            reason: e,
        })
...
mod tests {
    ...
    fn test_read_system_status_encode_decode() {
        let read_cmd = ReadSystemStatus::new(vec![(SystemType::FireAlarm, 0x12)]).unwrap();
    ...
    fn test_read_component_status_encode_decode() {
        let read_cmd =
            ReadComponentStatus::new(vec![(SystemType::FireAlarm, 0x12, 0x789ABC)]).unwrap();
