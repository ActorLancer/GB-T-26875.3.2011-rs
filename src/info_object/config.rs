...
impl InfoObject for FireSystemConfig {
    ...
    fn parse(data: &[u8]) -> ParseResult<Self> {
        ...
        Self::new(system_type, system_address, config_description, timestamp).map_err(|_| {
...
impl DeviceConfig {
    ...
    pub fn new(
        ...
        if device_description.len() > 255 {
            return Err(crate::error::ParseError::InvalidDataLength {
                actual: device_description.len(),
                expected: 255,
            });
        }
...
impl InfoObject for DeviceConfig {
    ...
    fn parse(data: &[u8]) -> ParseResult<Self> {
        ...
        Self::new(device_description, timestamp).map_err(|_| {
            crate::error::ParseError::InvalidDataLength {
                actual: device_desc_len,
                expected: 255,
            }
        })
    }
...
mod tests {
    ...
    fn test_device_config_encode_decode() {
        let timestamp = Timestamp::now();
        let config = DeviceConfig::with_description_text("GB26875用户信息传输装置", timestamp)
            .expect("Failed to create config");
    ...
    fn test_config_description_text() {
        ...
        .with_description_text("烟雾探测器配置");
        assert_eq!(
            component_config.description_text().unwrap(),
            "烟雾探测器配置"
        );

        let device_config =
            DeviceConfig::with_description_text("GB26875用户信息传输装置", timestamp)
                .expect("Failed to create config");
        assert_eq!(
            device_config.description_text().unwrap(),
            "GB26875用户信息传输装置"
        );
    }
}
